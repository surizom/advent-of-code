import { compact, maxBy, uniqBy } from "lodash";
...
const input = `---7|.|-7F7.-FF-77FF7J-L-77.JF-.FLJ-7.F77F7-J-L7LF7J.L7.-L-.F7-7F7-|-F---..FFF7-L|.|..-.|J.F-77--77|--J.LFLF7F-|7-|.LFF-FJ7FL--7F-FJ-J-L.FFJ
...
type Coord = { lineIndex: number; indexInLine: number };

const startingPointCoordinates = lines
  .map((line, lineIndex) => {
    const hasStartingPoint = line.includes("S");
    if (!hasStartingPoint) {
      return null;
    }
    ...
    const coord: Coord = {
      lineIndex,
      indexInLine: line.findIndex((c) => c === "S"),
    };
...
console.log(startingPointCoordinates);

const allPoints = lines.map((line, lineIndex) =>
  line.map((symbol, symbolIndex) => {
    const coord: PointInLoop = {
      lineIndex,
      indexInLine: symbolIndex,
...
const coordDiff =
  (coordDiff: Coord) => (comparisonOrigin: Coord, comparedPipe: Coord) =>
    comparedPipe.lineIndex - comparisonOrigin.lineIndex ===
      coordDiff.lineIndex &&
    comparedPipe.indexInLine - comparisonOrigin.indexInLine ===
      coordDiff.indexInLine;

const IS_IN_DIRECTION = {
  north: coordDiff({ indexInLine: 0, lineIndex: -1 }),
  south: coordDiff({ indexInLine: 0, lineIndex: 1 }),
  east: coordDiff({ indexInLine: 1, lineIndex: 0 }),
  west: coordDiff({ indexInLine: -1, lineIndex: 0 }),
} as const;

const PIPE_CONNEXIONS = {
  "|": ["north", "south"],
  "-": ["east", "west"],
  L: ["north", "east"],
  J: ["north", "west"],
...
type Symbol = keyof typeof PIPE_CONNEXIONS | "." | "S";

type PointInLoop = {
  symbol: Symbol;
} & Coord;

type PointInLoopWithDistance = PointInLoop & { distance: number };

const isConnected = (comparisonOrigin: Coord, comparedPipe: PointInLoop) => {
  const pipeSymbol = comparedPipe.symbol;

  const pipeConnexions:
    | (typeof PIPE_CONNEXIONS)[keyof typeof PIPE_CONNEXIONS]
    | null = pipeSymbol in PIPE_CONNEXIONS ? PIPE_CONNEXIONS[pipeSymbol] : null;

  if (!pipeConnexions) {
    return false;
  }

  const directionsResult = pipeConnexions.map((direction) =>
    IS_IN_DIRECTION[direction](comparedPipe, comparisonOrigin)
  );

  return directionsResult.some((d) => d);
};

const fmt = (point: Coord) => `x${point.indexInLine}y${point.lineIndex}`;

const evaluatedPoints = new Set(fmt(startingPointCoordinates));

const markAsTreated = (points: Coord[]) =>
  points.forEach((point) => evaluatedPoints.add(fmt(point)));

const hasNotAlreadyBeenLookedAt = (point: Coord) =>
  !evaluatedPoints.has(fmt(point));

const findNextPointsInLoop =
  (currentPointsInLoop: PointInLoopWithDistance[]) =>
  (potentialPointsInLoop: PointInLoop[]) => {
    const connectedPipes = compact(
      potentialPointsInLoop.map((point) => {
        const interestingPointsInLoop = currentPointsInLoop.filter(
          (pointInLoop) =>
            Math.abs(pointInLoop.indexInLine - point.indexInLine) < 2 &&
            Math.abs(pointInLoop.lineIndex - point.lineIndex) < 2
        );

        for (const pointInLoop of interestingPointsInLoop) {
          const isConnectedToPoint = isConnected(pointInLoop, point);

          if (isConnectedToPoint) {
            return {
              ...point,
              distance: pointInLoop.distance + 1,
            };
          }
        }
...
const pointsInLoop: PointInLoopWithDistance[] = [
  { ...startingPointCoordinates, symbol: "S", distance: 0 },
];

let steps = 0;

let lastfound = pointsInLoop;

const allSurroundingPoints =
  (allPoints: PointInLoop[][]) =>
  (point: PointInLoop): PointInLoop[] =>
    compact(
      [
        [1, 0],
        [0, 1],
        [-1, 0],
        [0, -1],
      ].map(([x, y]) => allPoints[point.lineIndex + y]?.[point.indexInLine + x])
    );

while (lastfound.length !== 0) {
  const next = findNextPointsInLoop(pointsInLoop)(
    uniqBy(
      pointsInLoop.map(allSurroundingPoints(allPoints)).flat(),
      (p) => p.indexInLine + "," + p.lineIndex
    )
  );
  ...
  const filteredNext = next.filter(
    (nextPoint) =>
      !pointsInLoop.some(
        (point) =>
          point.indexInLine === nextPoint.indexInLine &&
          point.lineIndex === nextPoint.lineIndex
      )
  );

  lastfound = filteredNext;

  pointsInLoop.push(...filteredNext);
  ...
  console.log(
    ...
    lastfound.length,
    pointsInLoop.length,
    allPoints.flat().length
  );
}

console.log(pointsInLoop);

const farthest = maxBy(pointsInLoop, (point) => point.distance);
