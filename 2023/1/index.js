...
const digitsByLine = input.split("\n").map((line)=>{
  const digits = []
  ...
  for(let i = 0; i < line.length; i++){
    ...
    const correspondingNumber = Object.keys(NUMBERS).find(number=>line.slice(i).startsWith(number))

    if(correspondingNumber){
      digits.push(NUMBERS[correspondingNumber])
    }

    
  }

  const sum = parseInt(digits[0] + digits.at(-1))
...
const result = digitsByLine.reduce((acc, i)=>acc+(i),0)
