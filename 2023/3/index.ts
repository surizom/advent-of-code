...
const symbolPositions = lines.flatMap((line, lineIndex) => {
  const symbolIndexes =
    [...line.matchAll(new RegExp("[^0-9.]", "g"))].map((value) => ({
      index: value.index,
      symbol: value[0],
    })) ?? [];

  return symbolIndexes.map(({ index, symbol }) => ({
    lineIndex,
    symbolIndex: index,
    symbol,
  }));
...
const numberWithPositions = lines
  .map((line, lineIndex) => {
    ...
    return numbersWithIndexes.map(({ index, value }) => ({
      lineIndex,
      numberIndex: index,
      number: value,
    }));
...
type SymbolPosition = { lineIndex: number; symbolIndex: number };

const isDigitAdjacentToSymbol = (
  symbolPosition: SymbolPosition,
  digitPosition: { lineIndex: number; numberIndex: number }
): Boolean => {
  return (
    Math.abs(symbolPosition.lineIndex - digitPosition.lineIndex) <= 1 &&
    Math.abs(symbolPosition.symbolIndex - digitPosition.numberIndex) <= 1
  );
};

const isNumberAdjacentToSymbol = (
  symbolPosition: { lineIndex: number; symbolIndex: number },
  numberWithPosition: { lineIndex: number; numberIndex: number; number: string }
): boolean => {
  const allNumberDigitsPositions = new Array(numberWithPosition.number.length)
    .fill(0)
    .map((_, digitIndex) => ({
      ...numberWithPosition,
      numberIndex: numberWithPosition.numberIndex + digitIndex,
    }));

  return allNumberDigitsPositions.some((digitPosition) =>
    isDigitAdjacentToSymbol(symbolPosition, digitPosition)
  );
};

const numbersAdjacentToSymbols = numberWithPositions.filter((number) =>
  symbolPositions.some((symbol) => isNumberAdjacentToSymbol(symbol, number))
);

const sum = numbersAdjacentToSymbols.reduce(
...
console.log({
  sum,
  numbersAdjacentToSymbolsLen: numbersAdjacentToSymbols.length,
  allNumbersLen: numberWithPositions.length,
  symbolsLen: symbolPositions.length,
});

// Part 2

const isSymbolGear = (
  symbolPosition: { lineIndex: number; symbolIndex: number; symbol: string },
  allNumbersWithPosition: {
    lineIndex: number;
    numberIndex: number;
    number: string;
  }[]
): {
  isSymbolGear: boolean;
  adjacentNumbers: number[];
  gearRatio: number | null;
} => {
  if (symbolPosition.symbol !== "*") {
    return { isSymbolGear: false, adjacentNumbers: [], gearRatio: null };
  }

  const allNumbersAdjacentToSymbol = allNumbersWithPosition.filter((number) =>
    isNumberAdjacentToSymbol(symbolPosition, number)
  );

  if (allNumbersAdjacentToSymbol.length !== 2) {
    return { isSymbolGear: false, adjacentNumbers: [], gearRatio: null };
  }

  const adjacentNumbers = allNumbersAdjacentToSymbol.map((number) =>
...
console.log({
  gearSymbolsLen: symbolPositions.filter(
    (symbol) => isSymbolGear(symbol, numberWithPositions).isSymbolGear
  ).length,
});

const sumOfGearRatios = symbolPositions.reduce(
  (acc, symbol) => acc + isSymbolGear(symbol, numberWithPositions).gearRatio,
  0
);
