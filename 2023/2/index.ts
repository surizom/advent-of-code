...
const games = cleanInput
  .split("\n")
  .map((line) => {
    const [game, results] = line.split(":");
    const id = parseInt(game.split(" ")[1]);

    const parsedGameResults = results
      .split(",")
      .map((res) => res.trim())
      .map((game) => {
        const quantity = parseInt(game.split(" ")[0]);
        const color = game.split(" ")[1] as "blue" | "green" | "red";

        if (!["blue", "green", "red"].includes(color)) {
          throw Error("invalid color: " + color);
        }

        if (!parseInt(game.split(" ")[0])) {
            throw Error("invalid qty: " + quantity);
        }
    ...
    return {
      id,
      results: parsedGameResults,
    };
  });

const reproducedInput = games.map(game=>`Game ${game.id}: ${game.results.map(res=>`${res.quantity} ${res.color}`).join(", ")}`).join("\n")

if(reproducedInput !== cleanInput){
    throw Error("discrepency")
}
...
const possibleGames = games.filter((game) => {
  const isBluePossible = game.results
    .filter(({ color }) => color === "blue")
    .every(({ quantity }) => quantity <= GAME.blue);

  const isGreenPossible = game.results
    .filter(({ color }) => color === "green")
    .every(({ quantity }) => quantity <= GAME.green);

  const isRedPossible = game.results
    .filter(({ color }) => color === "red")
    .every(({ quantity }) => quantity <= GAME.red);

  return isBluePossible && isGreenPossible && isRedPossible;
});

const sum = possibleGames.reduce((acc, current) => acc + current.id, 0);

const minimalGamesPerLine = games.map((game) => {
  const requiredNumberOfGreen = Math.max(
    ...game.results
      .filter(({ color }) => color === "green")
      .map(({ quantity }) => quantity)
  );

  const requiredNumberOfBlue = Math.max(
    ...game.results
      .filter(({ color }) => color === "blue")
      .map(({ quantity }) => quantity)
  );

  const requiredNumberOfRed = Math.max(
    ...game.results
      .filter(({ color }) => color === "red")
      .map(({ quantity }) => quantity)
  );
  ...
  return {
    id: game.id,
    red: requiredNumberOfRed,
    green: requiredNumberOfGreen,
    blue: requiredNumberOfBlue,
  } as const;
});

const powers = minimalGamesPerLine.map(
  (minimalGame) => minimalGame.blue * minimalGame.green * minimalGame.red
);

console.log(powers)

const sumPowers = powers.reduce((acc, current) => current + acc, 0);
